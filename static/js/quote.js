...
function loadProducts() {
    ...
    scheduled_function = setTimeout(ajax_func, delay_by_in_ms, quote_url, request_parameters);
}
...
$(window).on('load', function(){
    ...
    $(current_step).prevAll('.step').addClass('step-completed');
})
...
function validateCableLength(){
    if (!$("#cablelengthField").val()) {
        ...
        var input = document.getElementById("cablelengthField");
        localStorage.setItem(input.id, input.value);
        var url = new URL(window.location);
...
function confirmInvBatteryLength(){
    if (parseInt($("#cablelengthbtoinvField").val()) <= 0) {
        ...
        var input = document.getElementById("cablelengthbtoinvField");
        localStorage.setItem(input.id, input.value);
        var url = new URL(window.location);
